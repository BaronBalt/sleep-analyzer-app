...
class SleepsAdapter(
    ...
    override fun onBindViewHolder(holder: SleepViewHolder, position: Int) {
        ...
        holder.start.text = formatter.formatTimestamp(Date(sleep.start), preferencesRepository.compactView())
        holder.stop.text = formatter.formatTimestamp(Date(sleep.stop), preferencesRepository.compactView())
        val durationMS = sleep.stop - sleep.start
        val durationText = formatter.formatDuration(durationMS / 1000, preferencesRepository.compactView())
        ...
        if (preferences.getBoolean("show_awake_for", false)) {
            ...
            val durationWakeText = formatter.formatDuration(
                durationWakeMS / 1000,
                preferencesRepository.compactView()
            )
